<template>
  <div class="tab-menu pt2">
    <a-tabs :activeKey="route.name" hide-add type="editable-card" :animated="true" size="small" @tabClick="handleTabClick" @edit="handleTabEdit">
        ...
        <template #tab>
          <span class="flxsc">
            <template v-if="pane.icon">
              <img :src="pane.icon" style="width: 14px; height: 14px" alt="menu-icon" v-if="isHttp(pane.icon)" />
              <super-icon :name="pane.icon" :type="pane.icon.includes('svg-') ? 'svg' : 'antd'" size="14" v-else />
            </template>
